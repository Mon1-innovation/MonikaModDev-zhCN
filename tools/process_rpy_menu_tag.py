...
def process_line(line, line_number):
    ...
    Args:
        ...
        line_number (int): 行号
    """
    global is_menu_block, current_indent, menu_choices, current_label

    # 清除行尾换行符并去除首尾空白
    line = line.rstrip()

    # 检测label开始
    label_name = is_label_start(line)
    if label_name:
        current_label = label_name
        print(f"Entered label: {current_label}")

    # 检测menu开始
    if is_menu_start(line):
        is_menu_block = True
        current_indent = get_indent(line)
        menu_choices = []  # 重置选择项列表
        print(f"Menu found in label '{current_label}' at line {line_number}")
        return
        ...
        if is_choice_line(line):
            ...
            if choice_indent > current_indent:
                ...
                processed_choices.append({
                    ...
                    "tl_text": existing_translations.get(choice_text, default)["translation"],
                    'position': "# {}".format(current_label),
                }
                )

        # 检测menu块结束（通过缩进回到menu开始的缩进级别）
        if get_indent(line) <= current_indent and line.strip():
            # 输出当前menu的所有选择项
            if menu_choices:
                print(f"Menu choices in label '{current_label}':")
                for choice in menu_choices:
                    print(f"  Indent: {choice['indent']}, Text: {choice['text']}")
            ...
            if is_menu_block and is_choice_line(raw_line):
                ...
                if choice_indent > current_indent:
                    ...
                    if choice_text and "{#" not in choice_text:
                        ...
                        # 在最后一个双引号前插入tag
                        line_out = re.sub(r'"\s*:$', f'{tag}":', raw_line)
