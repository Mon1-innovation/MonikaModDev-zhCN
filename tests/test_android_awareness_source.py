...
class AndroidAwarenessSourceTests(unittest.TestCase):
    ...
    def test_awareness_module_defines_direct_java_bridge_and_reaction_helpers(self):
        ...
        self.assertIn("def mas_awareness_read_state(log_state=True)", source)
        ...
        self.assertNotIn("Java state raw_type=", source)
        self.assertNotIn("Java state parsed as dict successfully", source)
        self.assertNotIn("Java state was JSON-encoded twice", source)
        self.assertIn("if log_state:", source)
        self.assertIn("_mas_awareness_log(_mas_awareness_last_state_error)", source)
        self.assertIn('_mas_awareness_log("Error parsing Java state: " + str(e))', source)
        self.assertIn('_mas_awareness_log("Error reading Java state: " + str(e))', source)
    ...
    def test_dev_android_awareness_debug_topic_reports_runtime_values(self):
        ...
        self.assertIn("store.mas_awareness_read_state(log_state=False)", source)
        self.assertIn("def dev_android_awareness_get_active_window_from_state", source)
        self.assertNotIn("_mas_dev_android_debug_call(mas_getActiveWindowHandle", source)
        ...
        self.assertIn("不会写入 raw state 日志", source)
        self.assertNotIn("完整 raw state 已写入 mas_log", source)
